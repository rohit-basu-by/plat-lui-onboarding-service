...
export class UserCollabService {
    ...
    async createUser(username: string, tenant_context: TenantContext): Promise<RemoteUserDto> {
        ...
        let userCreated;
        try {
            userCreated = await this.httpService.post<RemoteUserDto>(`${tenant_context.COLLABORATION_HOST_URL}/users?iid=${''}`,
                newUser,
                {
                    headers
                        : {
                        'Authorization': 'Bearer '.concat(tenant_context.ADMIN_ACCESS_TOKEN),
                        'Content-Type': 'application/json'
                    }
                }
            ).toPromise();
